"""Benchmark the clean_ass style classifier against a directory of real .ass files.
...
summary with the key features (prose / positioned / styling fractions), plus the
styles that need a human decision (ASK) and any DROP with a dialogue-like name
(a possible regression). Files stay local; nothing is written.
"""
...
class StyleAgg:
    ...
    def add(
        self, decision: Decision, lines: int, share: float, prose: float,
        pos: float, sty: float,
    ) -> None:
    ...
    @property
    def avg_prose(self) -> float:
        return self.prose_sum / self.files if self.files else 0.0

    @property
    def avg_pos(self) -> float:
        return self.pos_sum / self.files if self.files else 0.0

    @property
    def avg_sty(self) -> float:
        return self.sty_sum / self.files if self.files else 0.0
...
def main() -> int:
    ...
    for path in files:
        try:
            stats = compute_style_stats(parse_ass(normalize_bytes(path.read_bytes())))
        ...
        for name, st in stats.items():
            ...
            agg[series][name].add(
                verdict.decision, st.line_count, st.line_share,
                st.prose_fraction, st.positioned_fraction, st.styling_fraction,
            )
    ...
    for series in sorted(agg):
        ...
        for name, sa in ordered:
            ...
            print(
                f"  {sa.decision.value.upper():4}  {name:22.22}  lines={sa.lines:<6} "
                f"prose~{sa.avg_prose:>3.0%} pos~{sa.avg_pos:>3.0%} sty~{sa.avg_sty:>3.0%}{mixed}"
            )
    ...
    for style in ask_styles:
        print(f"  - {style}")
    print(f"\nFlags (DROP with a dialogue-like name -> possible regression): {len(flags)}")
    for flag in flags:
        print(f"  - {flag}")
    return 0
