...
def scan_library(session: Session, library: Library) -> int:
    ...
    for path in sorted(Path(library.path).rglob("*")):
        ...
        episode = _get_or_create_episode(session, season, fields.episode or 1)
...
def _get_or_create_episode(session: Session, season: Season, number: int) -> Episode:
    existing = session.scalar(
