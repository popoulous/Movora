...
export function ActivityBell(): JSX.Element {
  ...
  const detail = runningTask
    ? (runningTask.message ?? (runningTask.progress > 0 ? `${runningTask.progress}%` : ""))
    : "";
  const indicator = runningTask
    ? `${taskLabel(runningTask, t)}${detail ? ` ${detail}` : ""}` + (queued > 0 ? ` (+${queued})` : "")
    : t("activity.working");
  ...
  return (
    <div className="relative flex items-center gap-2">
      {/* Running indicator: admins get a link to /tasks with details; viewers just see the spinner. */}
      {running && isAdmin && (
        <Link
          to="/tasks"
      ...
      {running && !isAdmin && (
        <span className="flex items-center gap-2 rounded-lg bg-white/5 px-2.5 py-1.5 text-xs text-neutral-200 ring-1 ring-white/10">
          <Loader2 className="h-3.5 w-3.5 animate-spin text-violet-300" />
          ...
          <div className="absolute top-full right-0 z-50 mt-2 max-h-[80vh] w-72 overflow-auto rounded-xl bg-[#120e1d] p-2 shadow-xl ring-1 ring-white/10">
            ...
            {!isAdmin ? (
              <p className="px-2 py-3 text-sm text-neutral-500">
                {running ? t("activity.working") : t("activity.empty")}
              </p>
            ) : tasks.length === 0 ? (
              ...
              <ul className="max-h-80 overflow-auto">
                {shown.map((task) => (
                  <li key={task.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm">
                    ...
                    {task.status === "running" && (task.message ?? task.progress > 0) && (
