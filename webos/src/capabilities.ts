...
// NOTE: there is no automatic audio probe. On this webOS build the Web Audio graph
// reads pure silence even while sound is clearly audible — createMediaElementSource
// is a no-op, and captureStream() yields no usable signal either. So "is there
// actually sound?" can't be measured here; the CapabilityView plays each audio
// sample and the user confirms by ear (Igen/Nem). has_audio is therefore reported
// from that answer, not from a measurement.
